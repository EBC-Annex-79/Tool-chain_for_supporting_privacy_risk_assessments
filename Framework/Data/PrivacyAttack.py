...
class PrivacyAttack:
    ...
    def get_privacy_risk_score(self):
        if self.privacy_risk_score is None:
            self.template_subject, self.output_data_type, self.output_subject, self.risk_description, self.privacy_risk_score  = self._find_output_for_template()
    ...
    def _find_output_for_template(self):
        ...
        q = rdflib.plugins.sparql.prepareQuery("""
                SELECT ?templateSubject ?outputSubject ?description ?privacyRiskScore
                WHERE {
        ...
        for row in ro:
            ...
            return row[0], self.PRIVVULN.PrivacyRisk, row[1], row[2], privacy_risk_score