...
def run_analyses(json):
    ...
    for node in nodes:
        ...
        if "superType" in node and node["superType"] != "" and node["superType"] and node["superType"] != str(
                PRIVVULNV2.Context):
        ...
        if "attributes" in node:
            for attribute in node["attributes"]:
                ...
                if attribute["dataType"] == "int" or attribute["dataType"] == "double" or attribute[
                    "dataType"] == "string":
                    if attribute["value"] != '' and attribute["value"] != None:
                        g1.add((subject, rdflib.term.URIRef(str(PRIVVULNV2) + attribute["name"]),
                                Literal(attribute["value"],
                                        datatype=rdflib.term.URIRef(str(XSD) + attribute["dataType"]))))
