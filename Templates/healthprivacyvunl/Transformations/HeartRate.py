...
class PhotoplethysmographyToHeartRate(ITransformation):
    ...
    def _build_model(self):
        ...
        photoplethysmographyToHeartRate = self.MODELS['PhotoplethysmographyToHeartRate']
        self.graph.add((photoplethysmographyToHeartRate, self.RDF.type, self.PRIVVULN.Transformation))
        self.graph.add((inputNode, self.PRIVVULN['feeds'], photoplethysmographyToHeartRate))

        heartRate = self.MODELS['HeartRate']
        self.graph.add((heartRate, self.RDF.type, self.PRIVVULN.TimeSeries))
        self.graph.add((heartRate, self.RDF.type, self.__DOMAINNAMESPACE__.HeartRate))
        self.graph.add((photoplethysmographyToHeartRate, self.PRIVVULN.feeds, heartRate))
