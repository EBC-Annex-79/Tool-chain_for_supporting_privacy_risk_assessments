...
class LimbMassAndLimbSurfaceAreaToMetabolicRate(ITransformation):
    ...
    def _build_model(self):
        ...
        self.graph.add((timeResolutionLinear1, self.PRIVVULNV2.TimeInput, Literal("1",datatype=self.XSD.double)))
        self.graph.add((timeResolutionLinear1, self.PRIVVULNV2.TimeOutput, Literal("1",datatype=self.XSD.double)))
        self.graph.add((inputNode2, self.PRIVVULN.feeds, timeResolutionLinear1))

        limbMassAndLimbSurfaceAreaToMetabolicRate = self.MODELS['LimbMassAndLimbSurfaceAreaToMetabolicRate']
        self.graph.add((limbMassAndLimbSurfaceAreaToMetabolicRate, self.RDF.type, self.PRIVVULN.Transformation))
        self.graph.add((inputNode, self.PRIVVULN['feeds'], limbMassAndLimbSurfaceAreaToMetabolicRate))
        self.graph.add((inputNode2, self.PRIVVULN['feeds'], limbMassAndLimbSurfaceAreaToMetabolicRate))

        metabolicRate = self.MODELS['MetabolicRate']
        self.graph.add((metabolicRate, self.RDF.type, self.PRIVVULN.TimeSeries))
        self.graph.add((metabolicRate, self.RDF.type, self.__DOMAINNAMESPACE__.MetabolicRate))
        self.graph.add((limbMassAndLimbSurfaceAreaToMetabolicRate, self.PRIVVULN.feeds, metabolicRate))